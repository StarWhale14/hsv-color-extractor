...
def extract_color_area(pil_img, selected_rgb, hue_tolerance, thickness,
                       sv_min_sat=DEFAULT_SV_MIN_SAT, sv_min_val=DEFAULT_SV_MIN_VAL, auto_thickness=False):
    ...
    kernel = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (max(1, thickness), max(1, thickness)))
...
st.title("🎨 HSV 기반 색상 추출기 (Web)")

uploaded_files = st.file_uploader("이미지 파일 선택 (여러 개 가능)", type=["png", "jpg", "jpeg", "bmp"], accept_multiple_files=True)
...
bg_rgba = {"White": (255, 255, 255, 255), "Gray": (128, 128, 128, 255), "Black": (0, 0, 0, 255)}[bg_color]
        ...
        with zipfile.ZipFile(result_zip, "w", zipfile.ZIP_DEFLATED) as zipf:
            ...
            for file in uploaded_files:
                ...
                result_img = extract_color_area(
                    ...
                    sv_min_sat=min_sat, sv_min_val=min_val, auto_thickness=auto_thick
                )
                bg_img = Image.new("RGBA", result_img.size, bg_rgba)
                bg_img.alpha_composite(result_img)
                with cols[0]:
                    st.image(pil_img, caption=f"원본: {file.name}", use_container_width=True)
                with cols[1]:
                    st.image(bg_img, caption=f"추출 결과: {file.name}", use_container_width=True)
                base_name = file.name.rsplit(".", 1)[0]
                buf = io.BytesIO()
                bg_img.save(buf, format="PNG")
                zipf.writestr(f"{base_name}_extract.png", buf.getvalue())
        result_zip.seek(0)
        st.download_button("📦 ZIP으로 다운로드", result_zip, file_name=f"extract_{datetime.now().strftime('%Y%m%d_%H%M%S')}.zip")

st.caption("© 2025 HSV 색상 추출 웹앱 (Streamlit 버전)")